...
usage：
    ...
    ./docker.sh rebuild       [--image-name NAME] [--cont-name NAME] [--dockerfile PATH]

parameter：
    -i, --image-name NAME     設定 image 名稱
    -c, --cont-name NAME      設定 container 名稱
    -u, --username USER       設定容器內要使用的使用者名稱
    -h, --hostname NAME       設定容器 hostname
    -m, --mount HOST[:CONT]   綁定目錄，可重複。若只給 HOST，則掛載到容器同路徑 HOST。
                                                例：--mount /home/me/work:/workspace  或  --mount /home/me/work
    -d, --dockerfile PATH     設定 Dockerfile 路徑（預設：./Dockerfile）

example：
    ./docker.sh run  -i aoc2026-env -c aoc2026-dev -u USER -h aoc2026
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -c|--cont-name)    CONT_NAME="${2:?}"; shift 2;;
        -u|--username)     USERNAME="${2:?}"; shift 2;;
        -h|--hostname)     HOSTNAME_DEF="${2:?}"; shift 2;;
